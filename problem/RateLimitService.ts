 ...
 * Também gerencia o "Panic Mode" e "Brute Force Protection" (Smart Lockout).
 */
export class RateLimitService {
    ...
    public static init() {
        try {
            ...
            // CORREÇÃO CRÍTICA DE TYPESCRIPT (e: any)
    ...
    // --- BRUTE FORCE PROTECTION (AUTH LOCKOUT) ---
    ...
    public static async checkAuthLockout(
        ...
        for (const key of lockKeys) {
            ...
            if (isLocked) {
                return { locked: true, reason: 'Too many failed attempts. Please try again later.' };
            }
        }
        return { locked: false };
    }
    ...
    public static async registerAuthFailure(
        ...
        // TTL para contagem de falhas (janela de observação). 
        // Ex: Se falhar 5 vezes em 1 hora.
        const failureWindowSeconds = 3600; 
    ...
    public static async clearAuthFailure(
        ...
        // Limpa todas as estratégias possíveis para garantir sanidade
        const keys = [
    ...
    public static async check(
        ...
        const limit = matchedRule ? matchedRule.rate_limit : 50; 
