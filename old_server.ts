...
// CRITICAL FIX: Dynamic CORS must run BEFORE resolveProject/hostGuard.
// This ensures CORS headers are present even on error responses (404/500), preventing net::ERR_FAILED.
const dynamicCors: RequestHandler = (req: any, res: any, next: any) => {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type,Authorization,apikey,x-cascata-client,Prefer,Range,x-client-info,x-supabase-auth,content-profile,accept-profile,x-supabase-api-version');
...
const cascataAuth: RequestHandler = async (req: any, res: any, next: any) => {
  ...
  // Fix: Explicitly allow access to GoTrue routes (/auth/v1/signup, /auth/v1/token, etc.)
  // even if no valid JWT is present, as long as the project context is resolved.
...
// FIX: Move dynamicCors UP. It must run BEFORE hostGuard to prevent 404 on OPTIONS.
...
app.get('/api/data/:slug/auth/v1/callback', async (req: any, res: any, next: NextFunction) => {
    ...
    try {
        ...
        // Se o state veio vazio (mesmo com a lógica do authorize), tenta fallback de novo aqui (dupla checagem)
        if (!finalRedirect) {
             const origins = r.project.metadata?.allowed_origins || [];
             if (origins.length > 0) {
                 const first = origins[0];
